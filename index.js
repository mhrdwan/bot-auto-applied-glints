  ...
  async function addLink() {
    ...
    if (fs.existsSync('listsemua.json')) {
      ...
      if (perikasa.length > 1) {
        console.log("File listsemua.json sudah ada dengan panjang data:", perikasa.length);
        gabungan = JSON.parse(perikasa);
        nomorUrut = gabungan[gabungan.length - 1].no + 1; 
      }
    }
  
    console.log(`Terdapat`, angkaTerakhir, `pagination dan`, angkaTerakhir * 30, `data`);
    ...
    fs.writeFileSync('listsemua.json', JSON.stringify(gabungan, null, 2), (err) => {
      if (err) {
